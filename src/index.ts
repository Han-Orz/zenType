...
export default class ZenType extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    try {
      ...
      if (__ZENTYPE_DEV__) {
        ...
        if (this.disposed) { this.debug.destroy(); this.debug = null; return; }
      }
      this.session = createWritingSession(this.features, this.debug ?? undefined);
      for (const name of ["typewriter", "ripple"] as const) {
        this.addCommand({
          langKey: "toggle-" + name,
      ...
      this.updateIcon();
      const refresh = () => {
        ZENTYPE_DEBUG: this.debug?.record("host", "eventbus-refresh");
        this.session?.refresh();
      };
      const suspend = () => {
        ZENTYPE_DEBUG: this.debug?.record("host", "eventbus-suspend");
  ...
  private updateFeatures(): void {
    ZENTYPE_DEBUG: this.debug?.record("session", "features-changed", {
    ...
    this.saving = this.saving.then(() => this.saveData("zentype-enabled", snapshot)).catch(error => {
      ...
      if (!this.disposed) showMessage("zenType：设置未能保存", 4000, "error");
    });
  }
  ...
  async onunload(): Promise<void> {
    ...
    this.session = null;
    if (__ZENTYPE_DEV__) { this.debug?.destroy(); this.debug = null; }
    this.style?.remove();
    this.style = null;
    this.topbar = null;
    await this.saving;
