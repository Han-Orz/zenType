...
export default class ZenType extends Plugin {
  ...
  private debug: DebugHookController | null = null;
  ...
  async onload(): Promise<void> {
    ...
    try {
      ...
      this.debug = initDebugHook();
      if (__ZENTYPE_DEV__) {
        await this.debug.start("default", { profile: "full", includeText: true });
        showMessage("zenType：DebugKit 已开始（全量记录）", 3000, "info");
        this.addCommand({
          langKey: "debugkit-toggle",
          langText: "zenType：切换 DebugKit",
          callback: () => { void this.toggleDebugKit(); },
        });
        this.addCommand({
          langKey: "debugkit-export",
          langText: "zenType：导出 DebugKit",
          callback: () => { void this.exportDebugKit(); },
        });
      }
      this.session = createWritingSession(this.features, this.debug);
      for (const name of ["typewriter", "ripple"] as const) {
        this.addCommand({
          langKey: "toggle-" + name,
      ...
      this.topbar = this.addTopBar({ icon: ICON, title: "zenType " + BUILD_LABEL, callback: () => this.toggleBoth() });
      this.updateIcon();
      const refresh = () => {
        this.debug?.record("host", "eventbus-refresh");
        this.session?.refresh();
      };
      const suspend = () => {
        this.debug?.record("host", "eventbus-suspend");
        this.session?.suspend();
      };
      this.eventBus.on("loaded-protyle-static", refresh);
      this.eventBus.on("loaded-protyle-dynamic", refresh);
      this.eventBus.on("switch-protyle", refresh);
      this.eventBus.on("destroy-protyle", refresh);
      ...
      this.off = [
        ...
        () => this.eventBus.off("switch-protyle", refresh),
        () => this.eventBus.off("destroy-protyle", refresh),
  ...
  private updateFeatures(): void {
    this.debug?.record("session", "features-changed", {
      typewriter: this.features.typewriter,
      ripple: this.features.ripple,
    });
  ...
  private async exportDebugKit(): Promise<void> {
  ...
  private async toggleDebugKit(): Promise<void> {
    const debug = this.debug;
    if (!debug) return;
    try {
