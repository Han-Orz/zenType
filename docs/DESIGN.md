...
**May know:** numerical state, targets and the selected feature response. Motion stays continuous across retargets; the block Ripple owner is a session-stepped CriticalState presented by an inert carrier effect.
...
Cursor-only interpolation and Ripple-only sentence fades reuse geometry. A normal lifecycle suspension retargets Ripple to neutral and lets the existing Critical Motion / Session render paths finish. Window blur keeps the Cursor presentation but shares that same Ripple release: a renderer that is still visible can animate, so owners retarget from their current value instead of being released in place. Only a hidden document tears the owners down synchronously, because it cannot show the release and must not keep a background loop. Disable, destruction and frame failure still clear synchronously. Self-authored scroll events matching the last actual write do not invalidate host facts. Host-driven scroll events do. After settling there is no continuous JavaScript loop; the existing delayed wake changes the cursor's bounded breathing target. Resize, fonts, viewport and theme events invalidate observations.

Explicit bounds: 256 mutation records and 2,048 visited nodes per classification, 64 ancestry levels, 48 semantic siblings per direction and at most 2,048 visited elements per block plan. Painting retains at most 2,048 owners between commits (a rebind moves an existing owner before any release, so no frame paints an unowned block). Replacement matching and handoff walk bounded owner/ancestor sets, not every owner/target pair. Ownership-budget overflow releases block owners instead of silently dropping part of the handoff.
...
`ripple/blockPlan.ts` contains two read-only boundaries: `collectTargets()` describes the disjoint neighborhood and semantic marker distance; `planHandoff()` maps prior **presented alpha** onto a changed owner plan. BlockPainter retains the bounded set of committed opacity ancestors on each owner and samples their state before any handoff writes. Live post-reparent ancestry cannot reconstruct that prior presentation: Tab can move an independently painted marker under a formerly dim sibling. An existing owner keeps its exact composite baseline; only a newly exposed dim target inherits the nearest released ancestor's presented value. Child-to-ancestor transfer takes the brightest donor and divides residual children by the incoming parent's baseline. Neutral focused targets do not acquire a new ancestor's dim. The wrapper is not promoted to an owner. Opacity still composites multiplicatively, and the existing partition and marker-distance policy remain unchanged.

`modules/structurePresentation.ts` is the presentation owner of bounded local layout continuity: the surviving semantic elements the Host is about to displace slide from the position the user last saw to the geometry the Host has already committed. `capture()` runs once before the Host mutation and reads one rect for a bounded local window — the edit's anchor (the focused NodeListItem, otherwise the focused block) and its following semantic siblings, at most 24 — because those are the only elements whose position the edit can change. `attach()` runs **inside the mutation delivery**, not at `geometry-ready`, because a frame that paints the Host's new layout before the transform exists is exactly the jump this exists to avoid. A captured identity that did not survive is dropped; a same-key replacement is matched and takes over the running state so a re-render continues one displacement instead of starting a second. The live Selection may provide the capture anchor, and `visualKey` remains a semantic validation. A Host or theme that owns its own inline transform is never overwritten: it fails the whole capture closed, and a sibling that already owns its own transform never joins the window. A capture whose own generation ends without acting is abandoned, while a displacement already running keeps converging. `step()` advances every surviving displacement on the shared Session frame with `MOTION.structuralMoveResponseMs` — a displacement is its own personality dimension, not the block alpha timescale — and one frame may advance at most a quarter of the response so a stalled renderer cannot teleport a subject. Reduced motion and sub-pixel displacements never attach. It is not a FLIP pass: no document snapshot, no placeholder, no clone, no height animation, no new-block fade, no deleted-node ghost, no second structural tree. Enter lets following content yield, Delete and Backspace/merge let surviving content reclaim its place, and Tab/Shift+Tab keep their reparent continuity through the same bounded window.

The caret is attached to the glyphs, so Session presents the offset of the caret's own subject on the caret while storing the Host's untransformed `frame.caret`; the Structural Contract therefore still reads stable geometry. A displaced sibling moves a subject element, not the caret, so it never transports the cursor. Cursor consumes its own subject's offset through `carried`, which places the position instead of approaching it: one physical displacement has exactly one motion owner, and a second critical motion toward the same offset would only make the cursor trail the glyphs. Cursor keeps its own Critical Motion for every other intent, including structural caret moves that no displacement owns.
...
`ripple/blockPainter.ts` owns semantic presentation state and bindings. An owner is keyed by `visualKey` (or the element when the Host provides no key); the bound element is only its current paint actuator. Replacement matching uses the owner's saved semantic key, including disconnected predecessors and same-id ancestors, and a match moves the same state — value and velocity preserved — to the new actuator instead of cancelling and rebuilding a motion. There is no separate carryover registry. On bounded mutation delivery it rebinds an existing owner to a same-key added element while preserving its semantic target and committed opacity ancestry; if detached sentence highlights were the only dim owner, Ripple hands their current visible floor to that same semantic block. It does not run the neighborhood planner against intermediate DOM. Full target planning/handoff runs only at an admitted commit or normal focus/binding change; no readiness level writes an owner. A Session frame only advances bound CriticalStates and updates their actuators: it never calls `collectTargets`, traverses topology, reads `getComputedStyle` or plans.
...
Running block motion pauses during the gate by holding each state where it is; resuming continues from that value and velocity. Sentence motion also stops advancing because Session does not render it. Native DOM layout is not frozen: host-driven text movement, live Range changes and compositing during reparenting remain integration risks. A covering dim owner released for focused-subtree safety does not uncover anything it was dimming: its composite is re-expressed on every off-path sibling from that element down to the focused block, in the same delivery, so exactly the focus path stops being multiplied and the role the user saw continues instead of a dim -> bright -> dim gap.
...
Block painting never writes `element.style.opacity`, `commitStyles()`, a dim class, an opacity custom property or a transition override. Block alpha is semantic presentation state — a CriticalState with `value`/`velocity`/`target` and a semantic key — advanced by the Session frame through the same critical law as Cursor and Sentence Ripple. The only pixel actuator is one short, **paused**, linear KeyframeEffect per owner whose `currentTime` encodes the state (`alpha = base * (1 - currentTime / 1000ms)`); only the painter's own write sets it, so no CSS `transition` on `opacity` can lengthen a fade and no WAAPI `play()`/`finish()`/`cancel()` lifecycle decides motion. The effect keeps an exact id and remains an Animation object, so the serialization argument below is unchanged. Each owner has at most one carrier. A rebind to a replacement element moves the same state to a new actuator; value and velocity continue, so nothing is cancelled and rebuilt. An owner converges to `target = 1` and is released; a dim owner stays as the source of its presented alpha for an ancestry handoff. Normal blur/lifecycle suspension retargets owners to neutral on the ordinary Session frame; disable, editor switch, timeout, exception handling and destruction release them.
...
Acquisition samples host baseline opacity once per binding; the carrier's keyframes encode that base and the underlying host inline/CSS value is never overwritten. A composite alpha cannot be decomposed from computed opacity alone, so an inherited or host-authored opacity is never mistaken for an ancestor factor. Release reveals current host style, including changes by another integration; host opacity changes during ownership are revealed on release/reacquisition rather than polled.
