...
**Owns:** bounded input intent, MutationRecord classification, generation supersession, structural evidence, identity readiness, geometry readiness, semantic readiness, deadline/overflow release, and one per-transaction `StructuralHandoff`.

**May know:** the Host frame and bounded mutation summary supplied by Session, the monotonic execution time, and semantic block keys. The handoff is intentionally minimal: `generation`, `topologyChanged`, `fromBlockKey`, `toBlockKey`, `identityReady`, `geometryReady`, and `semanticReady`. `fromBlockKey` is the semantic block at the last trustworthy caret when the transaction starts; `toBlockKey` is the destination block in the new authoritative frame. A different key is evidence for features to interpret, not an animation prescription.
...
The three readiness levels are distinct Host facts and are never merged. `identityReady` says the current generation has observed a structural topology change and the authoritative Host frame already resolves a credible focused semantic block, so focus/caret ownership has moved from `fromBlockKey` to `toBlockKey`; it says nothing about caret x/y/height stability, neighborhood stability or animation state, and it does not wait for two stable samples. `geometryReady` keeps its existing meaning: continuous trustworthy caret geometry. `semanticReady` keeps its existing meaning: geometry ready plus 48ms of structural quiet. Identity publication is once per generation (`identityPublished`), reset by new activity or a new generation; an unavailable frame, a range selection, a caretless block or a block without a semantic key fails closed and publishes nothing.
...
**May know:** all five-layer outputs needed to route authority, including the Structural Contract decision/handoff and lifecycle state. Session routes `CursorIntent` (`"typing"`, `"navigation"`, or `"structural"`) but never selects a motion curve. Identity-ready transfers only the focused block owner; geometry-ready grants Cursor; Typewriter and full Ripple semantics stay held until semantic-ready.
...
| identity-ready | hold | hold | focused block only |
| geometry-ready | yes | hold | focused block only |
...
4. With evidence, the first authoritative caret frame that resolves a semantic block establishes `identity-ready` and publishes it once: focus ownership has moved to `toBlockKey`. Two matching caret samples (position/height within 0.15px and the same block binding) then establish `geometry-ready`; missing geometry, a non-caret selection or a caretless block breaks the stable run. Identity-ready may transfer only the newly authoritative focused block presentation owner for a verified cross-key handoff; geometry-ready authorizes Cursor. Full Ripple neighborhood and sentence semantics still require at least 48ms without relevant activity. These are provisional readiness heuristics, not a SiYuan completion signal.
...
Session uses its existing rAF only during bounded evidence recovery. Intent-only waiting sleeps on the existing wake until classified activity, the ordinary candidate's 48ms quiet boundary, or the absolute 160ms deadline. Once structural evidence exists, Session samples on that same rAF even through missing caret geometry. An identity-ready sample transfers at most the newly authoritative focused block owner — before that frame's presentation is committed, so the first authoritative merged topology is never presented with the previous dim owner. A geometry-ready sample updates Cursor; Typewriter, sentence Ripple and the remaining block neighborhood stay held until semantic commit. This allows Cursor to follow `null → valid A → valid A` or repeated structural targets without turning intermediate topology into a commit. Every run terminates at the original deadline; no recurring idle sampling is introduced. Reduced motion changes animation, not authority: it must still pass this gate.
