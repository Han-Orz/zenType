...
Input establishes writing intent; browsing cancels automatic scrolling. Composition belongs to an editor. Editor switches discard old effect bindings. Lifecycle and DOM events invalidate observations; none certifies host completion on its own.
...
`ripple/blockPainter.ts` owns animations and bindings. Replacement matching uses the previous owner's saved semantic key, including disconnected predecessors and same-id ancestors. There is no separate carryover registry. During a withheld frame it can rebind an existing owner to a same-key added element and hold its alpha; it does not run the neighborhood planner against intermediate DOM. New target planning/handoff runs only at an admitted commit or normal focus/binding change.
...
Deterministic Session tests now cover text evidence preceding delayed structural evidence, the 48ms ordinary confirmation path, intent-only deadline release, missing-caret structural recovery, range/pointer/wheel/lifecycle/editor-switch interruption, and withholding of Cursor, Typewriter and Ripple preparation. Painter tests cover exact-id stale WAAPI recovery without host animation or inline-opacity mutation. The Chromium marker fixture remains available but is not required in CI: this environment has neither Docker nor a local Chromium/SiYuan host. Real SiYuan validation of MutationObserver/frame order, merge geometry, IME, delayed replacements, clipping and theme interactions remains outstanding; automated fixtures do not turn the heuristic thresholds into an official host completion contract.
