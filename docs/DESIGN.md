...
`ripple/structuralMove.ts` owns the bounded structural geometry handoff: one Host element slides from the position the user last saw to the geometry the Host has already committed. `capture()` runs once before the Host mutation and reads a single rect for the focused item; `begin()` runs at the first `geometry-ready` frame and refuses a mismatched `visualKey`, so DOM identity never substitutes for semantic continuity; `step()` advances on the shared Session frame with the same critical law and the same `blockFadeMs` timescale as block alpha, writing a `translate()` and restoring whatever inline transform the element had. Reduced motion and sub-pixel displacements snap. It is not a FLIP pass: no document snapshot, no placeholder, no clone, no second structural tree. The caret is attached to the glyphs, so Session presents the same offset on the caret while storing the Host's untransformed `frame.caret`; the Structural Contract therefore still reads stable geometry.
