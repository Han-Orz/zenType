...
**May know:** all five-layer outputs needed to route authority, including the Structural Contract decision/handoff and lifecycle state. Session routes `CursorIntent` (`"typing"`, `"navigation"`, or `"structural"`) but never selects a motion curve. Geometry-ready grants Cursor and, for a verified cross-key topology handoff, only the new focused block owner; Typewriter and full Ripple semantics stay held until semantic-ready.
