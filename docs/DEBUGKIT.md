...
### v2.9.0-remake.2.6 Structure Identity Handoff
...
- `structure-sample`：intent 阶段记录 input/non-structural observation 与等待/ordinary 原因；evidence 阶段记录 caret、quiet、stableFrames、`identityReady`、`geometryReady`、`semanticReady` 与等待/geometry/commit 原因。
- `structure-identity-ready`：同一 generation 首次在权威 Host frame 上解析出可信语义 block identity；只授权 Ripple 的 focused block ownership，不代表 caret geometry 稳定或 Cursor/Typewriter/semantic commit 已获得授权。payload 为 `generation`、`topologyChanged`、`fromBlockKey`、`toBlockKey`。
- `structure-geometry-ready`：同一 generation 的连续可信 caret geometry 首次达到两次稳定采样；只授权 Cursor，不代表 Ripple/Typewriter 已获得 semantic commit。结构样本带有 `topologyChanged`、`fromBlockKey`、`toBlockKey`、`identityReady`、`geometryReady` 和 `semanticReady`。
