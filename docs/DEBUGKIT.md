...
- `structure-identity-ready`：同一 generation 首次在权威 Host frame 上解析出可信语义 block identity。它只发布证据，不授权任何 presentation 写入，也不代表 caret geometry 稳定或 Cursor/Typewriter/semantic commit 已获得授权。payload 为 `generation`、`topologyChanged`、`fromBlockKey`、`toBlockKey`。
...
本版删除了 `v2.9.0-remake.2.6-structure-identity.1` 的 `focused-identity-handoff` 实验事件及其 `hadDestinationOwner` 字段：identity-ready 不再对 destination 做任何 opacity 操作，因此没有对应的 presentation 事件可记录。期望顺序为 `structure-evidence` → `structure-identity-ready`（`geometryReady` 仍为 false）→ `structure-geometry-ready`（Cursor 接管 target）→ `structure-stable` / `ownership-commit`。
...
期望顺序：`structure-evidence` → `structure-identity-ready`（此时 `geometryReady` 仍为 false）→ `structure-geometry-ready`（Cursor 接管 target）→ `structure-stable` / `ownership-commit`。identity-ready 只发布证据；focused owner 的变更只发生在 semantic commit 的 ownership plan 中。
