...
Smooth cursor, typewriter scrolling and ripple focus for SiYuan. remake.2.1 keeps one WritingSession/rAF/observer system and adds a shared structural authority boundary; real SiYuan acceptance is still pending.
...
Potential structural edits withhold new Cursor, Typewriter and Ripple targets until host evidence is either confirmed ordinary or quiet, stable structural geometry is observed. Input alone never disproves structural intent. Recovery is bounded at 160ms and ordinary text deletion keeps a next-sample fast path after non-structural mutation evidence.
...
Disable zenType, back up the existing installation outside the plugins directory, extract the archive into your SiYuan workspace's `data/plugins/zenType/`, then enable it. Do not run two copies. The topbar tooltip identifies `v2.9.0-remake.2.1`; the numeric plugin manifest version is `2.9.0`.
